...
class ConsultationDetailsViewController: UIViewController
{
    ...
    @IBOutlet var consultName: UILabel!
    ...
    {
        performSegue(withIdentifier: "EditConsultation", sender: model)
    }
    ...
    func consultationDelMethod()
    {
        if CheckNetwork.isExistenceNetwork()
        {
            ...
            let urlStr = String(format : "%@?consultationId=%@",API.Consultation.ConsultationDelete, (self.model?.consultId)!)
            ...
            let task = session.dataTask(with: request)
            {
                    ...
                    let status = json?["Status"] as? Int
                {
                    ...
                    if status == 1
                    {
                        let alertDel = UIAlertController(title: nil, message: "Consultation details has been deleted successfully", preferredStyle: .alert)
    ...
    {
        if segue.identifier == "EditConsultation"
        {
            let edit: AddEditConsultationViewController = segue.destination as! AddEditConsultationViewController
