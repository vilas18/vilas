//
//  AddEditSerumElectrolytesViewController.swift
//  Active4PetsPMS
//
...
class AddEditSerumElectrolytesViewController: UIViewController, UITextFieldDelegate
{
    
    var model: CbgHemHemSerumModel?
    
    @IBOutlet var measureDate: UITextField!
    @IBOutlet var serumValue: UITextField!
    ...
    override func viewDidLoad()
    {
        ...
        self.serumValue?.layer.masksToBounds = true
        self.serumValue?.layer.borderWidth = 2
        self.serumValue?.layer.cornerRadius = 3.0
        self.serumValue?.layer.borderColor = UIColor.clear.cgColor
        
        
        self.urlStr = API.SerumElectrolytes.SerumElectrolyteAdd
        self.serumValue.keyboardType = .numberPad
        ...
        if self.isFromVC == true
        {
            ...
            self.serumValue.text = list[0]
            
            self.urlStr = API.SerumElectrolytes.SerumElectrolyteEdit
        }
    ...
    @objc func viewTouched(_ sender: Any)
    {
        ...
        serumValue?.resignFirstResponder()
    ...
    {
        if self.measureDate?.text == nil && (self.measureDate?.text == "") && self.serumValue?.text == nil && (self.serumValue?.text == "")
        {
            navigationItem.rightBarButtonItem?.isEnabled = true
            self.measureDate?.layer.borderColor = UIColor.red.cgColor
            self.serumValue?.layer.borderColor = UIColor.red.cgColor
        ...
        if self.serumValue?.text == nil || (self.serumValue?.text == "")
        {
            self.serumValue?.layer.borderColor = UIColor.red.cgColor
            self.mandatoryLbl?.isHidden = false
        }
    ...
    func webServiceToUpdateAdd()
    {
        ...
        if internetStatus != NotReachable
        {
            ...
            if self.isFromVC == true
            {
                message = "Serum electrolytes details has been updated successfully"
                str = String(format: "SerumElectroId=%@&HMT_TypeId=%@&MeasuredDate=%@&SerumElectrolyteValue=%@", (self.model?.commonId)!, "8", self.measureDate.text!, self.serumValue.text!)
            }
            else if self.isFromVC == false
            {
                message = "Serum electrolytes details has been added successfully"
                let petId:String = UserDefaults.standard.string(forKey: "SelectedPet")!
                str = String(format: "PetId=%@&HMT_TypeId=%@&MeasuredDate=%@&SerumElectrolyteValue=%@", petId, "8", self.measureDate.text!, self.serumValue.text!)
    ...
    func textFieldDidEndEditing(_ textField: UITextField)
    {
        if textField == serumValue
        {
            if textField.text == nil || textField.text == ""
            {
                ...
                {
                    self.serumValue?.layer.borderColor = UIColor.red.cgColor
                    let alert = UIAlertController(title: "Warning", message: "Serum Electolyte must be between 0 and 999 ", preferredStyle: .alert)
                ...
                {
                    serumValue?.layer.borderColor = UIColor.clear.cgColor
                }
            }
        }
    }
