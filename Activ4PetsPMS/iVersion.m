...
static NSString *const iVersionAppLookupURLFormat = @"http://itunes.apple.com/US/lookup?id=954829197";
//static NSString *const iVersionAppLookupURLFormat = @"http://itunes.apple.com/%@/lookup";

static NSString *const iVersioniOSAppStoreURLFormat = @"https://itunes.apple.com/us/app/activ4pets/id%@";
static NSString *const iVersionMacAppStoreURLFormat = @"macappstore://itunes.apple.com/app/id%@";
//static BOOL useAppStoreDetailsIfNoPlistEntryFound = YES;
...
- (iVersion *)init
{
    if ((self = [super init]))
    {
        ...
        NSString *tempCode = [(NSLocale *)[NSLocale currentLocale] objectForKey:NSLocaleLanguageCode];
        NSLog(@"tempCode:%@",tempCode);
        NSLog(@"self.appStoreCountry:%@",self.appStoreCountry);
        ...
        NSLog(@"applicationVersion:%@",self.applicationVersion);
        ...
        NSLog(@"applicationBundleID:%@",self.applicationBundleID);
        //default settings
...
//    return _updateAvailableTitle ?: [self localizedStringForKey:iVersionUpdateAvailableTitleKey withDefault:@"New version available"];
    return _updateAvailableTitle ?: [self localizedStringForKey:iVersionUpdateAvailableTitleKey withDefault:@"Would you like to upgrade to new version of Activ4Pets"];
}
...
#if TARGET_OS_IPHONE
    NSLog(@"app url : %@",[NSURL URLWithString:[NSString stringWithFormat:iVersioniOSAppStoreURLFormat, @(self.appStoreID)]]);
...
- (void)checkForNewVersionInBackground
{
    @synchronized (self)
    {
        @autoreleasepool
        {
            ...
            //first check iTunes
            NSString *iTunesServiceURL =  [NSString stringWithFormat:iVersionAppLookupURLFormat];
//             NSString *iTunesServiceURL = [NSString stringWithFormat:iVersionAppLookupURLFormat, self.appStoreCountry];
            
            if (self.appStoreID)
            {
                iTunesServiceURL = iTunesServiceURL ;//[iTunesServiceURL stringByAppendingFormat:@"?id=%@", @(self.appStoreID)];
            }
            else
            {
...
- (BOOL)openAppPageInAppStore
{
    ...
    if (self.verboseLogging)
    {
        ...
        //http://itunes.com/app/YourAppNameWithoutSpaces
        [[UIApplication sharedApplication] openURL: [NSURL URLWithString:@"http://itunes.com/app/activ4pets"]];
    return YES;
}
