...
class AllergyDetailsViewController: UIViewController
{
    ...
    @IBOutlet var allergyName: UILabel!
    ...
    override func viewDidLoad()
    {
        ...
        let edit = UIBarButtonItem(image: UIImage(named: "ic_edit"), style: .done, target: self, action: #selector(self.editAllergy))
        let del = UIBarButtonItem(image: UIImage(named: "trash.png"), style: .done, target: self, action: #selector(self.deleteAllergy))
    ...
    @objc func editAllergy(_ sender: Any)
    {
        performSegue(withIdentifier: "EditAllergy", sender: model)
    }
    
    @objc func deleteAllergy(_ sender: Any)
    {
    ...
    func allergyDelMethod()
    {
        if CheckNetwork.isExistenceNetwork()
        {
            ...
            let urlStr = String(format : "%@?allergyId=%@",API.Allergy.AllergyDelete, (self.model?.allergyId)!)
            ...
            let task = session.dataTask(with: request)
            {
                    ...
                    let status = json?["Status"] as? Int
                {
                    ...
                    if status == 1
                    {
                        let alertDel = UIAlertController(title: nil, message: "Allergy details has been deleted successfully", preferredStyle: .alert)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        ...
        {
            let edit: AddEditAllergyViewController = segue.destination as! AddEditAllergyViewController
