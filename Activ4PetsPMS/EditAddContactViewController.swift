//
//  EditAddContactViewController.swift
//  Active4PetsPMS
//
...
class EditAddContactViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate, UIPickerViewDelegate, UIPickerViewDataSource
{
    ...
    @IBOutlet weak var relationType: UITextField?
    ...
    override func viewDidLoad()
    {
        ...
        self.relationType?.layer.masksToBounds = true
        self.relationType?.layer.borderWidth = 2
        self.relationType?.layer.cornerRadius = 3.0
        self.relationType?.layer.borderColor = UIColor.clear.cgColor
        
        self.urlStr = API.Contact.ContactAdd
        self.relationType?.text = ""
        ...
        self.relationId = ""
        self.commentTxtfld?.text = "Enter comments"
        ...
        if self.isFromVC == true
        {
            ...
            self.urlStr = API.Contact.ContactEdit
            if model != nil
            {
                self.relationType?.text = self.model?.relation as String?
                self.fstNameTxtfld?.text = self.model?.fstName as String?
                self.lstNameTxtfld?.text = self.model?.lstName as String?
                ...
                self.relationId = (self.model?.relationId)!
        ...
        else if self.isFromEmerg || self.isFromdetails
        {
            ...
            if emgModel != nil
            {
                self.relationType?.text = self.emgModel?.relation as String?
                ...
                self.countryTxtfld?.text = self.model?.countryName
                self.countryId = self.model?.countryId ?? "0"
                ...
                self.relationId = self.emgModel?.relationId ?? "0"
    ...
    @objc func viewTouched(_ sender: Any)
    {
        ...
        relationType?.resignFirstResponder()
        ...
        if internetStatus != NotReachable
        {
            ...
            RestClient.getList(dropDownType, callBackHandler: {(response: Any, error: Error?) -> Void in
                ...
                DispatchQueue.main.async(execute: {() -> Void in
                    ...
                    {
                        if (self.dropDownType == "ContactType") {
    ...
    {
        if textField == self.countryTxtfld || textField == self.stateTxtfld || textField == self.relationType
        {
            let view1 = UIView()
    ...
    {
        if self.relationType?.text == nil && (self.relationType?.text == "") && self.fstNameTxtfld?.text == nil && (self.fstNameTxtfld?.text == "") && self.lstNameTxtfld?.text == nil && (self.lstNameTxtfld?.text == "")
        {
            navigationItem.rightBarButtonItem?.isEnabled = true
            self.fstNameTxtfld?.layer.borderColor = UIColor.red.cgColor
            self.relationType?.layer.borderColor = UIColor.red.cgColor
            self.lstNameTxtfld?.layer.borderColor = UIColor.red.cgColor
            self.mandatoryLbl?.isHidden = false
        }
        
        if self.relationType?.text == nil || (self.relationType?.text == "")
        {
            self.relationType?.layer.borderColor = UIColor.red.cgColor
            self.mandatoryLbl?.isHidden = false
        }
        ...
        if self.fstNameTxtfld?.text == nil || (self.fstNameTxtfld?.text == "")
        {
            self.fstNameTxtfld?.layer.borderColor = UIColor.red.cgColor
            self.mandatoryLbl?.isHidden = false
        }
        ...
        if self.lstNameTxtfld?.text == nil || (self.lstNameTxtfld?.text == "")
        {
            self.lstNameTxtfld?.layer.borderColor = UIColor.red.cgColor
            self.mandatoryLbl?.isHidden = false
        }
    ...
    func webServiceToUpdateAdd()
    {
        ...
        if internetStatus != NotReachable
        {
            ...
            if self.isFromVC == true || self.isFromEmerg == true || self.isFromdetails == true
            {
                ...
                if self.isFromVC
                {
                    contId = (self.model?.contactId)!
                }
                else if self.isFromEmerg || self.isFromdetails == true
                {
                    contId = (self.emgModel?.contactId)!
                }
                message = "Contact has been updated successfully"
                str = String(format : "ContactId=%@&UserId=%@&RelationshipId=%@&FirstName=%@&LastName=%@&IsEmergencyContact=%@&StartDate=%@&EndDate=%@&Address1=%@&Address2=%@&City=%@&CountryId=%@&StateId=%@&Zip=%@&Home=%@&Office=%@&Cell=%@&Fax=%@&Email=%@&Comments=%@", contId, userId, self.relationId, (self.fstNameTxtfld?.text)!, (self.lstNameTxtfld?.text)!, self.isEmergency, (self.startDate?.text)!, (self.endDate?.text)!, (self.address1Txtfld?.text)!, (self.address2Txtfld?.text)!, (self.cityTxtfld?.text)!, self.countryId, self.stateId, (self.zipcodeTxtfld?.text)!, (self.homeTxtfld?.text)!, (self.officeTxtfld?.text)!, (self.cellTxtfld?.text)!, (self.faxTxtFld?.text)!, (self.emailTxtfld?.text)!, (self.commentTxtfld?.text)! )
            }
            else if self.isFromVC == false
            {
                ...
                str = String(format: "PetId=%@&UserId=%@&RelationshipId=%@&FirstName=%@&LastName=%@&IsEmergencyContact=%@&StartDate=%@&EndDate=%@&Address1=%@&Address2=%@&City=%@&CountryId=%@&StateId=%@&Zip=%@&Home=%@&Office=%@&Cell=%@&Fax=%@&Email=%@&Comments=%@", petId, userId, self.relationId, (self.fstNameTxtfld?.text)!, (self.lstNameTxtfld?.text)!, self.isEmergency, (self.startDate?.text)!, (self.endDate?.text)!, (self.address1Txtfld?.text)!, (self.address2Txtfld?.text)!, (self.cityTxtfld?.text)!, (self.countryId), (self.stateId), (self.zipcodeTxtfld?.text)!, (self.homeTxtfld?.text)!, (self.officeTxtfld?.text)!, (self.cellTxtfld?.text)!, (self.faxTxtFld?.text)!, (self.emailTxtfld?.text)!, (self.commentTxtfld?.text)!  )
    ...
    func textFieldDidBeginEditing(_ textField: UITextField)
    {
        ...
        else if textField == relationType
        {
            textFld = textField
            dropDownType = "ContactType"
            self.listArr = []
            self.listTv.reloadAllComponents()
            getList()
            let toolbar = UIToolbar()
    ...
    @objc func doneDropDownSelection()
    {
        if listArr.count > 0
        {
            ...
            else if (dropDownType == "ContactType")
            {
                let model = listArr[self.listTv.selectedRow(inComponent: 0)] as? CommonResponseModel
                relationType?.text = model?.paramName
                relationId = (model?.paramID)!
            }
